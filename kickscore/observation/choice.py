...
def _choice_expectations(
    ...
    probs = _softmax(z)
    exp_ll = float(mean[winner] - np.mean(_logsumexp(z, axis=1)))
    pbar = np.mean(probs, axis=0)
    curv = np.mean(probs * (1.0 - probs), axis=0)
    grad = -pbar
    grad[winner] += 1.0
    return exp_ll, grad, curv
...
class PlackettLuceObservation(Observation):
    ...
    def kl_update(self, lr: float = 0.3) -> float:
        ...
        exp_ll, grad, tau_utility = _choice_expectations(mean, var, self._winner, self._eps)
    ...
    def probability(
        ...
        if integrate:
            ...
            z = mean[None, :] + np.sqrt(np.maximum(var, 0.0))[None, :] * eps
            probs = np.mean(_softmax(z), axis=0)
        else:
            probs = _softmax(mean[None, :])[0]
